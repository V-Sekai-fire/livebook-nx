...
def load_module_weights(
    ...
    load_epoch = ckpt["epoch"] if "epoch" in ckpt else 0
    load_global_step = ckpt["global_step"] if "global_step" in ckpt else 0

    return state_dict_to_load, load_epoch, load_global_step
...
class TimeRecorder:
    ...
    def end(self, name: str, accumulate: bool = False) -> float:
        ...
        spuv.info(f"{name}: {t:.2f}{self.time_unit}")
    ...
    def get_accumulation(self, name: str, average: bool = False) -> float:
        ...
        spuv.info(f"{name} for {len(acc)} times: {t:.2f}{self.time_unit}")
