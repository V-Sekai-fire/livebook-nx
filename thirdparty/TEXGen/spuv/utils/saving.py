...
class SaverMixin:
    ...
    def save_img_sequence(
        ...
        if name and self._wandb_logger:
            spuv.warn("Wandb logger does not support video logging yet!")
        return save_path

    def save_img_sequence_from_tensors(
            self,
            filename,
            tensor_list,
            save_format="mp4",
        ...
        imgs = [self.tensor_to_image(tensor) for tensor in tensor_list]
    ...
    def tensor_to_image(self, tensor):
        # Assuming tensor is in RGB format and normalized between 0 and 1
        # Convert tensor to a numpy array and ensure it's in the correct format and range for imageio
        image = tensor.detach().cpu().numpy()  # Adjust this line based on your tensor format
    ...
    def save_img_sequences(
        ...
        for f in os.listdir(seq_dir_):
            ...
            try:
                ...
                spuv.warn(f"Video saving for directory {seq_dir_} failed!")
