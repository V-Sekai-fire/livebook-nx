...
class Mesh:
    ...
    def remove_outlier(self, outlier_n_faces_threshold: Union[int, float]) -> Mesh:
        if self.requires_grad:
            spuv.debug("Mesh is differentiable, not removing outliers")
            return self
    ...
    def subdivide(self):
        if self.requires_grad:
            spuv.debug("Mesh is differentiable, not performing subdivision")
            return self
    ...
    def post_process(self, options):
        if self.requires_grad:
            spuv.debug("Mesh is differentiable, not performing post processing")
            return self
    ...
    def _unwrap_uv_xatlas(
        ...
        spuv.info("Using xatlas to perform UV unwrapping, may take a while ...")
