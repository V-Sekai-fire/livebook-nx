...
def bake_texture(
    ...
    if mode == 'fast':
        ...
        # Final clamp to ensure texture is in valid [0, 1] range before conversion
        with torch.no_grad():
            texture.data.clamp_(0.0, 1.0)
...
def to_glb(
    ...
    if textured:
        ...
        if textured:
            ...
            # Using 100 views for high-quality texture baking with 2K resolution
            observations, extrinsics, intrinsics = render_multiview(app_rep, resolution=2048, nviews=100)
