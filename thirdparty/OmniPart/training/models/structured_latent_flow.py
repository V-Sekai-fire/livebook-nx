...
class SparseResBlock3d(nn.Module):
    ...
    def forward(self, x: sp.SparseTensor, emb: torch.Tensor) -> sp.SparseTensor:
        ...
        # print(f"number of points in the output: {h.coords.shape[0]}")
...
class SLatFlowModel(nn.Module):
    ...
    def __init__(
        ...
        if pe_mode == "ape":
            ...
            self.layer_pe = nn.Embedding(self.part_max_size + 1, model_channels)  # +1 for overall object
            
        self.layer_pe_proj = nn.Linear(model_channels, model_channels)

        # Mask embedding
        self.dinov2_hidden_size = 1024
        self.group_emb_dim = 128

        self.group_embedding = nn.Embedding(self.part_max_size + 1, self.group_emb_dim)  # +1 for background

        self.group_emb_proj = nn.Linear(self.group_emb_dim, self.dinov2_hidden_size)
    ...
    def initialize_weights(self) -> None:
        ...
        nn.init.zeros_(self.layer_pe_proj.weight)
        nn.init.zeros_(self.layer_pe_proj.bias)

        # Initialize layer positional embeddings
        self.layer_pe.weight.data.normal_(mean=0.0,std=0.02)

        # Initialize group embedding
        nn.init.zeros_(self.group_emb_proj.weight)
        nn.init.zeros_(self.group_emb_proj.bias)
    ...
    def forward(self, x: sp.SparseTensor, t: torch.Tensor, cond: torch.Tensor, **kwargs) -> sp.SparseTensor:
        ...
        masks = kwargs['ordered_mask_dino']  # [b, h, w]
        ...
        masks_emb = self.group_embedding(masks)  # [b, h*w, 128]
        masks_emb = self.group_emb_proj(masks_emb)  # [b, h*w, 1024]
        ...
        # Update coordinates with part-wise batch IDs
        # x = x.replace(feats=x.feats, coords=torch.cat([new_batch_ids.view(-1, 1), x.coords[:, 1:]], dim=1))
        ...
        # h = h.replace(feats=h.feats, coords=torch.cat([new_transformer_batch_ids.view(-1, 1), h.coords[:, 1:]], dim=1))
        h = sp.SparseTensor(
            feats = x.feats,
        ...
        # Restore part-wise batch IDs for upsampling
        ...
        # h = h.replace(feats=h.feats, coords=torch.cat([original_batch_ids.view(-1, 1), h.coords[:, 1:]], dim=1))
        h = sp.SparseTensor(
            feats = h.feats,
